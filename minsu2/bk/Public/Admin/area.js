...
function change(v){
	var str="0";
	for(i=0;i<v;i++){
		document.getElementById(s[i]).selectedIndex = Math.max(document.getElementById(s[i]).selectedIndex, index[i]);
...
dsy.add("0_0",["北京市"]);
dsy.add("0_1_0",["和平区","河东区","河西区","南开区","河北区","红桥区","塘沽区","汉沽区","大港区","东丽区","西青区","津南区","北辰区","武清区","蓟州区","蓟县","宁河县","芦台镇","静海县","静海镇"]);
dsy.add("0_1",["天津市"]);
...
var opt0 = ["北京市","北京市","朝阳区"];//初始值
var index = new Array();
function _init_area(){  //初始化函数
	for(i=0;i<s.length-1;i++){
	  document.getElementById(s[i]).onchange=new Function("change("+(i+1)+")");
	}
	index[0] = isHasElementOne(dsy.get("0"),opt0[0]);
	index[1] = isHasElementOne(dsy.get("0_"+index[0]),opt0[1]);
	index[2] = isHasElementOne(dsy.get("0_"+index[0]+"_"+index[1]),opt0[2])+1;
	index[0]++;
	index[1]++;
	change(0);
}
