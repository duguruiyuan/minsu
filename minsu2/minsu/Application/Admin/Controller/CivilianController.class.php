...
* 房屋管理控制器
*/
class CivilianController extends AuthController {
	//房屋添加首页
	public function index()
	{	
		$result = M('civilianinfo')->SELECT();
	...
	public function add() {
		if (IS_POST) {
			...
			$civilianinfo = M('civilianinfo');
			$civilianimg = M('civilian_pic');
			//组合地址
			if ($_POST['s_county'] != '市、县级市') {
	...
	public function edit($id) {	
		...
		$civilianimg = M('civilian_pic');
		if (IS_POST) {
			$lidAll = $civilianimg->field('id')->where("lid={$id}")->select();
			//重组数组 取出id
			$temp = array();
			...
			foreach ($_POST['temp_img'] as $k => $v) {
				$hiid = $civilianimg->field('id')->where("pic='{$v}'")->find();
				$img_id[] = $hiid['id'];
			}
			...
			if ($in) {
				$civilianimg->where(array('lid'=>array('IN',$in)))->delete();
			}
			//上传图片
			$info = $this->upload();
			if ($info) {
			...
			$civilianinfo->where("id={$id}")->save($_POST);
			$this->success('编辑成功',U('index'));
		}else{
			$result = $civilianinfo->where("id={$id}")->find();
			$result_img = $civilianimg->where("lid={$id}")->select();
	...
	public function del($id) {
		$z = M('civilianinfo')->where("id={$id}")->delete();
		if ($z) {
			$this->success('删除成功');
		}else{
			$this->error('删除失败');
	...
	public function houseAudit()
	{
		...
		$result = M('civilianinfo')
					->JOIN("LEFT JOIN ms_userinfo ON ms_civilianinfo.userinfo_uiid=ms_userinfo.uiid")
					->where($where)
					->order('add_time desc')
					->SELECT();
		$this->assign('result',$result);
	...
	public function auditAction($status,$hid)
	{
		...
		switch ($status) {
			case 1:
				$z = $civilianinfo->where("hid={$hid}")->save(array('status'=>1));
				break;
			case 2:
				$z = $civilianinfo->where("hid={$hid}")->save(array('status'=>2));
				break;
		}
		if ($z) {
	...
	public function auditReason()
	{
		if(IS_POST){
			...
			$hid = I('post.hid');
			$civilianinfo = M('civilianinfo');
			$z = $civilianinfo->where("hid={$hid}")->save(array('status'=>3,'not_reason'=>$_POST['not_reason']));
			if($z){
				$this->success('操作成功',U('houseAudit'));
			}
		}else{
	...
	{
		$c = M('civilianinfo')->where("id={$id}")->save(array('is_top'=>1));
		if ($c) {
			$this->success('置顶成功');
		}else{
			$this->error('置顶失败');
	...
	{
		$c = M('civilianinfo')->where("id={$id}")->save(array('is_top'=>0));
		if ($c) {
			$this->success('成功取消');
		}else{
			$this->error('取消失败');
	...
	public function charge($id) {
		$c = M('civilianinfo')->where("id={$id}")->setField('contact_charge',1);
		if ($c) {
	...
	public function uncharge($id) {
		$c = M('civilianinfo')->where("id={$id}")->setField('contact_charge',0);
		if ($c) {
	...
	public function tag($id, $tag) {
		$c = M('civilianinfo')->where("id={$id}")->setField('tag',$tag);
		if ($c) {
			$this->success('标记成功');
		}else{
			$this->error('标记失败');
	...
	{
		$this->assign('result',M('civilianmessage')->where("lid={$id}")->select());
		$this->display();
	}
	...
	public function ajaxHousestats($dt=0) {
		...
		$click= array();
		if($dt==0) {
			$sql='SELECT DATE(createtime) as date, count(*) as click FROM ms_civilianinfo GROUP BY DATE(createtime)';
			$data=M()->query($sql);
			foreach ($data as $k => $v) {
				array_push($date,$v['date']);
				array_push($click,$v['click']);
			...
			$week=(M()->query($weekSql));
			$week=$week[0]['week'];
			$sql="SELECT WEEK(createtime,1)as date, count(*) as click FROM ms_civilianinfo WHERE DATE_FORMAT(createtime,'%y')=DATE_FORMAT(now(),'%y') GROUP BY WEEK(createtime,1)";
			$data=M()->query($sql);
			foreach ($data as $k => $v) {
				$dateDisplay =$week-$v['date'];
				if($dateDisplay==0) {
			...
			$sql='SELECT substring(createtime,1,7) AS date,count(*) as click FROM ms_civilianinfo GROUP BY substring(createtime,1,7)';
			$data=M()->query($sql);
			foreach ($data as $k => $v) {
				array_push($date,$v['date']);
				array_push($click,$v['click']);
